...
Shoal data are derived from the work of \citet{Cosby2010}, however a review of shoals bordering domain boundaries with the Florida Keys and Everglades identified numerous deficiencies ignoring well-established flow paths between the boundaries and model basins.  These shoals were adjusted and calibrated to produce water level dynamics inside the appropriate basins consistent with observed water level data. 
...
%----------------------------------------------------------------
%----------------------------------------------------------------
The physical basis of BAM is exceedingly simple.  Mass-transport over shoals is governed by transport velocity $v$ integrated over shoal depth and length
\begin{equation}
   v = \sqrt{2 g \, \frac{h_u - h_d}{1 + f}}
   \label{equ:Shoal Velocity}
\end{equation}
where $h_u$ and $h_d$ are upstream and downstream basin water levels, and $f$ a friction factor
\begin{equation}
  f = 2 g \, n^2 \, w \, \rho^{-4/3}
...
The mass flux of dissolved substances over each shoal $M$ ($\mathrm{g}$) is calculated as the product of the concentration of the substance in the water $C$ ($\mathrm{g \, m^{-3}}$) and the water mass flux $F$ ($\mathrm{m^{3} \, s^{-1}}$) across the shoal.  The mass fluxes $M$ are summed around the boundary of each basin, the net mass flux over all shoals is multiplied by the time step and, along with other inputs and outputs of mass during the time step, added to the mass in the basin at the beginning of the time step. The new mass is divided by the water volume to estimate the concentration at the end of the time step. 
...
\begin{figure}[H]
\begin{center}
  \includegraphics[ width = 1\textwidth ]{graphics/EastFLBayRainFilled.png}
...
\begin{figure}[H]
\begin{center}
  \includegraphics[ width = 1\textwidth ]{graphics/WestFLBayRainFilled.png}
...
Estimation of total rain over basins tens to hundreds of square kilometers in area is notoriously challenging based on sparse, point measurements.  To afford user control over selection and effective areal input of rain, a mapping of basin to rain gauge stations is specified in the basin parameters (\texttt{-bp, [--basinParameter]}) file.  An excerpt of the basin parameter file is shown here:
\small
\begin{verbatim}
...
The EDEN stage file used to drive runoff is specified with the \texttt{-bR [--basinStageRunoff]} option with a default value of \texttt{EDEN\_Stage\_OffsetMSL.csv}.  The mapping of EDEN stage values to coastal basins and the corresponding shoals is specified with the \texttt{-bS \newline [--basinStageRunoffMap]} with a default of \texttt{Basin\_Runoff\_Boundary.csv}.  Runoff from the hydraulic stage relationship can be disabled with the \texttt{-nR [--noStageRunoff]} option which has a default of \texttt{False}.  An example of the EDEN stage to coastal basin runoff mapping file is shown below.
...
BAM interpolates hourly tidal timeseries to the model timestep at runtime based on cubic spline fits to the hourly data.  The cubic splines are generated during model initialization and this process can be time consuming.  Two approaches are used to minimize this delay.  First, the spline generations are parallel processed with up to 4 processors (if available).  Second, subsets of hourly tidal data can be specified in the \texttt{basinTide} file that correspond to the model simulation period, say 2010-1-1 to 2015-12-31 instead of the entire model period of record.
...
Finally, the mean sea level timeseries is converted from the NAVD datum reference to mean sea level in Florida Bay over the 2008-2015 period.  This is necessary since BAM operates on water level anomalies rather than a geodetic reference.  The reference mean sea level in Florida Bay is determined from an average of daily mean water levels at stations LM, PK and MK over 2008-2015 smoothed with a 30-day moving average.  The resultant sea level signal is shown in figure \ref{fig:MSL}.
...
%----------------------------------------------------------------
Tidal and Everglades boundary basins impose water levels that interact with basins along the periphery of Florida Bay.  There are also two methods to impose stage or flow boundaries onto interior basins: fixed boundary conditions impose a constant stage or flow, dynamic boundary conditions impose values from a timeseries. 
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\begin{figure}[H]
  \includegraphics[ width = 1\textwidth ]{graphics/BAM_GUI.png}
  \caption{BAM graphical user interface.}
  \label{fig:BAM_GUI}
\end{figure}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\vspace{-1em}
The main window is divided into 6 sections:\\[-2em]
\begin{enumerate}
  \itemsep-2pt
  \item Menu bar
  \item Basin listbox
...
\texttt{Salt mass} and \texttt{Computed} salinity are from independent spreadsheet calculations.  Salinity verification can be found in \texttt{etc/BlueBankSalinity-Q+1000cms\_dt1.ods}.
